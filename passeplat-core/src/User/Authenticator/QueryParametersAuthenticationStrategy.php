...
class QueryParametersAuthenticationStrategy extends AuthenticationStrategyBase
{
    const QUERY_STRING__PP_TOKEN = 'PP_TOKEN';
    const QUERY_STRING__PP_USER_ID = 'PP_USER_ID';
    ...
    {
        if ($this->hostChecker->getHostTypeFromUrl($request->getUri()) !== HostCheckerInterface::HOST_TYPE__BASE) {
            return null;
        }
        ...
        if (empty($queryParameters[static::QUERY_STRING__PP_USER_ID])) {
            // Not even a user ID is available. Nothing more to do here.
            return null;
        }

        $userId = $queryParameters[static::QUERY_STRING__PP_USER_ID];
